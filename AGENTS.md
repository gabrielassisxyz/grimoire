...
- **Extraction:** `uv run --group extract python tools/extract_runes.py <install> <wiki>` regenerates the catalog records read out of an installed game. The `extract` group holds UnityPy and is deliberately not `dev`: it is a third-party parser for a third-party binary, the advisor must never import it, and CI has no game install to point it at. Everything under `tools/` runs offline against the reader's own licensed copy and prints normalized records; nothing it reads is ever tracked.
- **Test:** `uv run pytest`.
