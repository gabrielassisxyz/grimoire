...
- **Extraction:** `uv run --group extract python tools/extract_runes.py <install> <wiki> <retrieved>` regenerates the catalog records read out of an installed game. `tools/extract_skills.py` takes the same three arguments and does the same for skills, emitting only those the wiki dump names; the ones it skips are listed on stderr, and that list is the gap to close by capturing pages rather than by deriving a name from an identifier. The `extract` group holds UnityPy and is deliberately not `dev`: it is a third-party parser for a third-party binary, the advisor must never import it, and CI has no game install to point it at. The retrieval date is an argument rather than a constant, because it describes the wiki dump and not the run. Everything under `tools/` runs offline against the reader's own licensed copy and prints normalized records; nothing it reads is ever tracked.
- **Test:** `uv run pytest`.
...
- **Five classes of evidence, and every catalog record names its own.** `game_asset` (read from a packaged install: asset path plus build id), `game_screen` (read off the game's own interface: the fixture it came from, that fixture's window geometry, and the build id printed in the frame), `save_file` (read out of the player's own save: the domain, the rotation slot and write counter that identify the payload, and the build id), `community_source` (wiki, spreadsheet, guide: URL plus retrieval date plus the game version it describes), `measured` (a controlled in-game experiment: the procedure, the before state, the after state). Each record also carries a `confidence`. A record with no evidence is not a record.
- **A record's evidence is a list, and the loader enforces what each class owes.** The pairs worth trusting most are the ones two unrelated readings agree on, so a schema holding one class per record could only note the second in a comment, where nothing can weigh it and no check can find it. The required fields per class are not advisory: a `community_source` without its URL and retrieval date fails to load, because a citation nobody can follow is the failure the provenance rules exist to prevent, and a rule enforced only by prose is one the tree drifts away from quietly.
- **`save_file` exists because reading the save is the mechanism behind most of what the catalog knows, and it fitted none of the other classes.** An identifier only ever appears in the save, so every join between a name and an identifier has one foot there; stretching `game_asset` would confuse the player's own data with the publisher's shipped files, and reading a file that already exists is not the controlled experiment `measured` describes. Leaving it out did real damage while it lasted: the weapon anchor and every effect join in the pilot pack were recorded with only their weaker half typed, so the strongest evidence in the pack was the part no check could reach and no staleness rule could version.
- **`game_screen` exists because the game's own interface outranks every community source and had nowhere to be recorded.** A screenshot is not a packaged asset and reading a static screen is not an experiment, so the other three classes each fit it badly, and stretching one of them would have made a print indistinguishable from an extracted asset in a record that has to be weighed later. It is cited rather than merely asserted: the fixture is in the repository's ignored cache under the name the record gives, so the claim can be checked by opening it, and the frame carries the build id in its corner, so the record's version is read off the same image rather than remembered.
...
| **The extractor's output is not the tracked catalog, and overwriting one with the other destroys evidence.** `runes.toml` carries what only a person can add: `measured` records from controlled in-game experiments, `game_screen` tooltip citations, prose effects, and confidences raised above what an extraction can claim. The extractor re-derives the rest and knows none of it. Diff the run against the tracked file and port only the fields that actually changed. | tripwire | `test_no_rune_is_joined_to_an_achievement_the_game_has_superseded` catches one instance of the general problem, nothing catches the rest |
