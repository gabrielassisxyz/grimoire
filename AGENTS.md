...
- **Four classes of evidence, and every catalog record names its own.** `game_asset` (read from a packaged install: asset path plus build id), `game_screen` (read off the game's own interface: the fixture it came from, that fixture's window geometry, and the build id printed in the frame), `community_source` (wiki, spreadsheet, guide: URL plus retrieval date plus the game version it describes), `measured` (a controlled in-game experiment: the procedure, the before state, the after state). Each record also carries a `confidence`. A record with no evidence is not a record.
