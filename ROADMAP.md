...
- **A target build and the catalog it refers to.** The pilot build is stored as identifiers, and the catalog joins those to the names the game displays. Every reference the pilot build makes resolves, and a test holds that. Beyond the pilot the catalog is empty, and it reports what it does not have rather than guessing.
...
**Reading the save is its own class, `save_file`, and adding it closed a hole the other four had left.** An identifier only ever appears in the save, so every join between a displayed name and an identifier has one foot there, and until the class existed that foot went into a comment. A save reading cites the write it came from rather than the file: the ten slots of a domain are a ring the game writes round, so a slot alone names a position that has since been overwritten while the counter names the payload.
...
**Every reference the pilot build makes now resolves, and the last two took an experiment each.** The build asks for Skill Inclination: Electric. No identifier containing "Inclination" appeared anywhere in the save, while `RuneAffinityElectric` did, sitting in the exact versatility slot the build wants, in a preset that otherwise matched it. Two readings fitted: the interface's Inclination is written as `RuneAffinityElectric`, or Inclination had simply never been written to this save. The spelling favoured the second, which is the reason not to settle on it, since the same catalog already holds `RuneStartWeaponSkill` for a rune the interface calls Weapon Expert.

Equipping the runes one at a time into a preset that held nothing decided both. Skill Affinity with the Electric type added `RuneAffinityElectric`; Skill Inclination with the Electric type then added `RuneInclinationElectric`, the first occurrence of that string anywhere in the save. Each time the payloads were compared byte by byte and the only other difference in the file was the write counter. The convention turned out to be regular, so a resemblance rule would have produced the same answer. That is the point rather than an argument against it: what makes the answer usable is that neither experiment came out the other way, and a rule that happens to be right is indistinguishable from one that is wrong until something checks.

**A test asserts the whole build against the catalog as shipped**, so a reference that stops resolving is reported by name rather than discovered downstream.

**Three runes turned out to be one rune with a type selector.** Skill Affinity, Skill Inclination and Skill Mastery each appear once in the interface and open a dialog of the fifteen skill types, and the game states that every type selected consumes one versatility slot. That is why the interface label cannot serve as a catalog display name: it names the family, while the save distinguishes `RuneAffinityElectric` from `RuneAffinityNature`. The qualified name the wiki uses is unique per identifier and is what the records carry.
