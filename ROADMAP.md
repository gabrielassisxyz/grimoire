...
**One of the pilot build's seven runes is still unresolved, and the experiment that failed to resolve it is why the other six can be trusted.** The build asks for Skill Inclination: Electric. No identifier containing "Inclination" appears in any slot of any domain of the save, while `RuneAffinityElectric` does and sits in the exact versatility slot the build wants, in a preset that otherwise matches it. Two readings fitted: the interface's Inclination is written as `RuneAffinityElectric`, or Inclination has simply never been written to this save. The spelling favoured the second, which is the reason not to settle on it, since the same file already holds `RuneStartWeaponSkill` for a rune the interface calls Weapon Expert.

Equipping Skill Affinity with the Electric type into a preset that held nothing decided it. The payload gained exactly one identifier, `RuneAffinityElectric`, and comparing the two payloads byte by byte the only other difference in the file is the write counter. So that identifier belongs to Affinity, the rune the build wants has an identifier nothing has yet observed, and the gap is a real absence rather than a vocabulary mismatch. The player does own the rune, so it is a catalog gap and not an ownership one.
