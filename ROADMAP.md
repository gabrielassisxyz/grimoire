...
**One of the pilot build's seven runes is still unresolved, and it is recorded as absent rather than approximated.** The guide asks for Skill Inclination: Electric, and no identifier containing "Inclination" appears in any slot of any domain of the save, while `RuneAffinityElectric` does and sits in the versatility slot the build wants. The spreadsheet and the wiki both give Skill Affinity and Skill Inclination as different runes unlocked at different prestige levels on the same character, so the question is not whether they differ but which display name that one identifier carries. Reading it off the spelling is the resemblance rule the resolver exists to refuse, so the pair stays unrecorded and the lookup fails by name. Settling it needs a controlled experiment rather than a better guess: equip the other rune of the pair, re-read the presets, and the identifier that appears is the one that was missing.
